...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < thread_count; i++) {
        /* (73) create thread with attribute thread_handle[i], executing the function Thread_work, with rank i */
        pthread_create(&thread_handles[i], NULL, Thread_work, (void *)i);
    }

    /* Wait for threads to complete. */
    for (i = 0; i < thread_count; i++) {
        /* (78) join corresponding threads */
...
void *Thread_work(void* rank) {
    ...
    switch (method) {
        case 2:
        /* (109) semaphore critical section to add local integral to total */
        sem_wait(&sem);
        total += my_int;
        sem_post(&sem);
        break;
    case 3:
        ...
        break;
    default:
        /* (119) mutex critical section to add local integral to total */
